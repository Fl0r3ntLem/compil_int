...
object Test2 extends App:
  ...
  println(s"Is 1 + 2 an int ? ${eval(BinaryExp(Plus, Number(1), Number(2)), Map()) === INT}")
  println(s"Is fun x -> 1 + 2 a function returning an int ? ${
    eval(
      Function(
        "x",
        BinaryExp(Plus, Number(1), Number(2))), Map()
    )
      === FUNCTION(TVar("X"), INT)
  }")
